...
import { 
  ...
  Target, AlertTriangle, Shield, LayoutDashboard
} from 'lucide-react';
...
const Portfolio = () => {
  // Get dynamic data from UserDataContext
  const { 
    userData,
    getPortfolioSummary, 
    getAssetAllocation, 
  ...
  // State for reactive data that updates when userData changes
  const [portfolioSummary, setPortfolioSummary] = useState(() => getPortfolioSummary());
  const [assetAllocation, setAssetAllocation] = useState(() => getAssetAllocation());
  const [monthlyData, setMonthlyData] = useState(() => getMonthlyData());
  const [investmentGoals, setInvestmentGoals] = useState(() => getInvestmentGoals());
  const [liabilities, setLiabilities] = useState(() => getLiabilities());
  const recentActivity = recentActivityData;

  // Debug log to see userData changes
  console.log('Portfolio render: Current userData.incomes.length:', userData.incomes.length);

  // Update portfolio data when userData changes
  useEffect(() => {
    console.log('Portfolio: userData changed, updating data. Income count:', userData.incomes.length);
    console.log('Portfolio: Full userData:', userData);
    console.log('Portfolio: Timestamp:', Date.now());
    const newPortfolioSummary = getPortfolioSummary();
    const newAssetAllocation = getAssetAllocation();
    const newMonthlyData = getMonthlyData();
    const newInvestmentGoals = getInvestmentGoals();
    const newLiabilities = getLiabilities();
    
    setPortfolioSummary(newPortfolioSummary);
    setAssetAllocation(newAssetAllocation);
    setMonthlyData(newMonthlyData);
    setInvestmentGoals(newInvestmentGoals);
    setLiabilities(newLiabilities);
    
    console.log('Portfolio: Updated portfolio summary:', newPortfolioSummary);
  ...
  return (
    ...
    >
      <PageHeader 
        ...
        subtitle="Track your investments and financial performance"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="text-xs text-gray-500 mb-2">Income sources: {userData.incomes.length}</div>
          <div className="mb-6">
            <motion.h1 
