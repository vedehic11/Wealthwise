...
import { LogOut, Settings, Shield, User as UserIcon, Camera, Mail, Phone, MapPin, Calendar } from 'lucide-react';
...
const Profile = () => {
  ...
  const initialLast = (displayName?.split(' ').slice(1).join(' ') || '').trim();
  ...
  const sidebarItems = [
    { icon: UserIcon, label: 'Personal Info', section: 'personal' },
  ...
  const handleProfileUpdate = async () => {
    try {
      const updated = { ...(storedUser || {}), name: `${firstName} ${lastName}`.trim() };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex justify-end space-x-4 pt-6 border-t dark:border-gray-700">
          ...
          <button onClick={handleProfileUpdate} className="px-6 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">Save Changes</button>
        </div>
      </div>
    );
  };
      ...
      <div className="space-y-6">
        <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
          <div className="flex items-center">
            ...
            <span className="text-sm text-yellow-800 dark:text-yellow-200">Two-factor authentication is enabled</span>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-12">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Sidebar */}
          <motion.div className="lg:col-span-3 space-y-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="space-y-2">
                {sidebarItems.map((item) => (
                  <button key={item.section} onClick={() => setActiveSection(item.section)} className={`w-full text-left px-4 py-3 rounded-lg flex items-center transition-all duration-300 ${activeSection === item.section ? 'bg-indigo-50 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-300 shadow-sm' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                    <item.icon className="h-5 w-5 mr-3" />
                    {item.label}
                  </button>
                ))}

              <button onClick={() => { localStorage.removeItem('userData'); navigate('/sign-in'); }} className="w-full mt-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-4 py-3 rounded-lg flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/30 transition-all duration-300">
          ...
          {/* Main Content */}
          <motion.div className="lg:col-span-9" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">{sidebarItems.find(item => item.section === activeSection)?.label}</h2>
              {renderContent()}
            </div>
          </motion.div>
        </div>
