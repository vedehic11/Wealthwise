...
import { BookOpen, GraduationCap, TrendingUp, Shield, Brain, PlayCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <PageHeader 
          title="Money Matters Learning Center"
          subtitle="Expand your financial knowledge with curated educational content"
          icon={BookOpen}
        />
        ...
        {selectedVideo && (
          <div className="mb-8 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <button onClick={() => setSelectedVideo(null)} className="mt-4 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300">Close Video</button>
          </div>
        )}
          ...
          {courses.map(({ level, icon: Icon }) => (
            ...
            >
              <div className="flex items-center justify-center">
                <Icon className={`h-6 w-6 ${selectedLevel === level ? 'text-indigo-500' : 'text-gray-400 dark:text-gray-500'}`} />
                <span className={`ml-2 font-medium ${selectedLevel === level ? 'text-indigo-700 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300'}`}>{level}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {courses.find((c) => c.level === selectedLevel)?.courses.map((course, index) => (
            <div key={index} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              ...
              <div className="mt-6">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div className="bg-indigo-600 h-2 rounded-full transition-all duration-300" style={{ width: `${course.progress}%` }} />
                </div>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                ...
                <button className="btn-secondary">{course.progress > 0 ? 'Continue' : 'Start Course'}</button>
              </div>
            </div>
          ))}

          {videos[selectedLevel]?.map((video, index) => (
            <div key={`video-${index}`} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              <div className="flex items-start space-x-4">
                <button onClick={() => setSelectedVideo(video.videolink)} className="flex-shrink-0 bg-indigo-100 dark:bg-indigo-900/50 rounded-lg p-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <button onClick={() => setSelectedVideo(video.videolink)} className="btn-secondary">Watch Now</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="relative">
              {[
                ...
                <div key={index} className="flex items-start mb-8 last:mb-0">
                  ...
                  <div className="ml-4">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">{step}</h3>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{index === 0 ? 'In Progress' : 'Upcoming'}</p>
                  </div>
                </div>
              ))}
            </div>
